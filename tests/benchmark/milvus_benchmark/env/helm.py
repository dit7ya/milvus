...
class HelmEnv(BaseEnv):
    ...
    def start_up(self, helm_path, helm_install_params):
        ...
        # update values
        values_file_path = f"{helm_path}/values.yaml"
        if not os.path.exists(values_file_path):
            raise Exception(f"File {values_file_path} not existed")
        lock_file_path = f"{helm_path}/values.yaml.lock"
        ...
        try:
            ...
            os.system(f"rm -rf {lock_file_path}")
            ...
            os.system(f"rm -rf {lock_file_path}")
            logger.error(f"Helm install server failed: {str(e)}")
            logger.error(traceback.format_exc())
            return False

    def tear_down(self):
        logger.debug(f"Start clean up: {self.name}.{self._name_space}")
